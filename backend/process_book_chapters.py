...
def read_book_content_from_markdown(file_path):
    """
    Read and chunk markdown files (including MDX) with structure preservation
    """
    with open(file_path, 'r', encoding='utf-8') as file:
        text = file.read()

    # Process the text with markdown-specific structure
    chunks = []

    # Look for markdown heading patterns (which are common in MDX/MD files)
    chapter_patterns = [
        r'(?:^|\n)#{1,6}\s+[^\n]+\n',  # Markdown headings
        r'(?:^|\n)###[^\n]*\n',        # H3 and above specifically
    ]
    ...
    for i, part in enumerate(text_parts):
        ...
        for j, sub_chunk in enumerate(sub_chunks):
            ...
            # Try to identify structure from content (look for headings in the original text)
            heading_match = re.search(r'#{1,3}\s+([^\n]+)', part[:500])
            heading = heading_match.group(1).strip() if heading_match else "unknown"

            chunks.append({
                "id": f"chapter-{Path(file_path).parent.name}-heading-{heading.replace(' ', '_').replace('#', '').strip()[:30]}-part{i}-sub{j}-{uuid.uuid4()}",
                "text": sub_chunk,
                "source_file": Path(file_path).name,
                "source_location": f"heading_{heading.replace(' ', '_').strip()}_part{i}_sub{j}",
                "metadata": {
                    "chapter_dir": Path(file_path).parent.name,
                    "heading": heading,
                    "part": i,
...
def process_specific_chapters():
    ...
    for chapter_dir in chapter_dirs:
        ...
        files = (
            ...
            list(chapter_path.glob("*.mdx")) +  # Add support for mdx files
            list(chapter_path.glob("*.md"))    # Add support for md files
        )
        ...
        for file_path in files:
            ...
            elif file_path.suffix.lower() in ['.mdx', '.md']:
                file_chunks = read_book_content_from_markdown(str(file_path))
            else:
