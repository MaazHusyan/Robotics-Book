...
def process_with_rate_limit(service, chunk, max_retries=3, base_delay=15):
    """
    Process a content chunk with rate limiting and retry logic
    For trial keys, we need to be very conservative with API calls
    """
    for attempt in range(max_retries):
        try:
            embedding = service.process_content_chunk(chunk)
            # Add a delay after successful processing to respect rate limits
            time.sleep(2)  # Wait 2 seconds between successful calls
            return embedding
        except Exception as e:
            if "429" in str(e) or "Rate Limit" in str(e):
                # Rate limit error - wait and retry with exponential backoff
                delay = base_delay * (2 ** attempt)  # Exponential backoff, starting with 15s
                print(f"       ⏰ Rate limit hit, waiting {delay}s before retry {attempt+1}/{max_retries}")
                time.sleep(delay)
                if attempt == max_retries - 1:  # Last attempt
                    print(f"       ❌ Failed after {max_retries} attempts: {str(e)}")
                    # Wait longer after a final failure before continuing to next chunk
                    time.sleep(5)
                    return None
            else:
                # Other error - don't retry
                print(f"       ❌ Error processing chunk: {str(e)}")
                # Wait before continuing to next chunk to avoid overwhelming
                time.sleep(1)
                return None
    ...
    for chapter_dir in chapter_dirs:
        ...
        for file_path in files:
            ...
            for i, content_data in enumerate(file_chunks):
                ...
                embedding = process_with_rate_limit(service, chunk)
                if embedding is not None:
                    print(f"       ✅ Stored in Qdrant: {embedding.chunk_id[:25]}... (dim: {embedding.dimensionality})")
                    total_chunks_processed += 1
                else:
                    print(f"       ⚠️  Skipped chunk due to processing error")
